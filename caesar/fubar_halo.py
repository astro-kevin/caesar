...
def fubar_halo(obj):
    """Dispatch to the appropriate halo-by-halo pipeline based on mode."""
    mode = resolve_mode(getattr(obj, "_kwargs", {}))

    if mode is Mode.AHF or mode is Mode.AHF_FAST:
        return _ahf_fubar_halo(obj)
    return _classic_fubar_halo(obj)


def _ahf_fubar_halo(obj):
...
def _classic_fubar_halo(obj):
    """Original FOF/SNAP halo+galaxy+cloud pipeline (upstream CAESAR)."""
...
def reset_global_particle_IDs(obj):
    ...
    if do_reset_check and 'galaxy' in obj.group_types:
        try:
            pre_gas_counts = [len(getattr(g, 'glist', [])) if getattr(g, 'glist', None) is not None else 0 for g in obj.galaxy_list]
        except Exception:
            pre_gas_counts = None
    ...
    if (do_reset_check or do_reset_assert) and pre_gas_counts is not None and 'galaxy' in obj.group_types:
        ...
        if do_reset_assert and pre_with > 0 and post_with == 0:
            mylog.error('Assertion: Gas lost after reset mapping (pre_with=%d, post_with=%d)', pre_with, post_with)
